...
```
TV-SALES-HOME (GitHub Organization)
│
├── demo_repository       ← Phase 1 Reference Implementation
...
├── crm                   (planned)
├── inventory             (planned)
└── infrastructure        (planned — Reusable Workflows)
```
...
Phase 1 uses `demo_repository` as the reference implementation to establish the DevOps standard. All future repositories will follow the same pipeline structure, adapted to their specific technology.
...
│   │   ├── cd-development.yml      ← Deploy to dev
